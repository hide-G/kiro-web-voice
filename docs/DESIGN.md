...
## 19. Tonal reset (v0.3.0)

初回リリース (v0.1.0) のアイコンは iOS 風のグラデーション角丸タイルで、Apple 製品と誤認される恐れがありました。同じく FAB も iOS Voice Memos を想起させる厚みでした。v0.3.0 では意図的にトーンを一段ニュートラルに寄せ、「ツール」として振る舞うようにしています。

### アイコン

| 旧 (v0.1.0〜0.2.0) | 新 (v0.3.0) |
| --- | --- |
| 角丸正方形 (iOS app tile) | 円 |
| Blue → Indigo リニアグラデ | 単色ダークチャコール `#2A2A2E` |
| トップ左に光沢ハイライト | なし |
| ドロップシャドウ | なし |
| system blue / indigo | ブランド中立 |

**残しているもの:** マイクの塗りグリフ、視認性を確保する強めのコントラスト、右上の録音キャパビリティを示す **控えめな緑ドット**（32 px 以上でのみ描画）。
...
| ラベル | 15 px | 12 px |
| ドット | 10 px | 7 px |
| radius | pill (継承) | pill (継承) |

半透明素材 (`backdrop-filter`) とスプリング挙動、`prefers-*` 対応は継承したままです。**"軽く/薄く" にした一方で、Apple Design 7 - Craft の観点で意匠は捨てていない** ことを明確にしています。

### 「読む」FAB の追加

出力側の音声動作を隠しコマンド (Alt+Shift+K) だけに委ねる設計は Agency (基礎原則 2) に反します。**入力の「話す」と対称に「読む」を配置** することで、以下を実現しました。

1. **可視化** — 音声出力が拡張の一等機能であることが一目でわかる。
2. **対称性 (Spatial consistency, 条項 7)** — 入力と出力を左右に並べる。
3. **中断可能性 (Interruptibility, 条項 3)** — 発話中に押すと即停止。速度・音声・スキップコード設定は既存ポップアップと共通。

### 読み上げ対象の解決

Kiro Web の DOM は動的で、ロールや属性の付き方が回によって異なります。単一の DOM セレクタに賭けず、**選択テキスト → 既知セマンティクス → クラス名ヒント → レイアウトヒューリスティック** の 4 段階で決定します。特に **選択テキストを最優先** にすることで、DOM に依存しない reliable な経路が常に確保されます。
